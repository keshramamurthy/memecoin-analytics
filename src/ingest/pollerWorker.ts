  ...
  async (job) => {
    ...
    try {
      ...
      if (
        ...
        error.message.includes('Invalid token mint')
      ) {
        console.error(
          `Invalid token detected: ${tokenMint} - removing from tracking`
        );
        await removeTokenFromTracking(tokenMint);
...
async function schedulePriceTrackingJobs(): Promise<void> {
  ...
  console.log(
    `Scheduling price tracking for ${validTokens.length} valid tokens (filtered out ${allTrackedTokens.length - validTokens.length} invalid)`
  );
...
worker.on('completed', (job) => {
  console.log(
    `Price update job ${job.id} completed for token ${job.data.tokenMint}`
  );
});

worker.on('failed', (job, err) => {
  console.error(
    `Price update job ${job?.id} failed for token ${job?.data.tokenMint}:`,
    err
  );
});
...
async function cleanupInvalidTokensFromDatabase(): Promise<void> {
  ...
  try {
    ...
    console.log(
      `✅ Database cleanup complete: removed ${cleanedCount} invalid tokens`
    );
  } catch (error) {
...
async function addTokenForTracking(tokenMint: string): Promise<void> {
  ...
  try {
    const { tokenValidationService } = await import(
      '../services/tokenValidationService.js'
    );
    const validation =
    ...
    if (!validation.isValid) {
      console.log(
        `🚫 REFUSED to track invalid token: ${tokenMint} - ${validation.reason}`
      );
    ...
    console.log(
      `🚫 REFUSED to track token due to validation error: ${tokenMint}`
    );
    return;
  }
...
async function obliterateTokenJobs(tokenMint: string): Promise<void> {
  ...
  try {
    ...
    for (const repeatableJob of repeatableJobs) {
      if (repeatableJob.name === jobId || repeatableJob.id === jobId) {
        try {
          ...
          const every =
            typeof repeatableJob.pattern === 'number'
          ...
          const repeatOptions = {
            ...
            ...(repeatableJob.endDate && {
              endDate: new Date(repeatableJob.endDate),
            }),
          };
    ...
    console.log(
      `☢️ OBLITERATION COMPLETE: ${tokenMint} (removed ${removedCount} jobs)`
    );
  } catch (error) {
...
async function removeTokenFromTracking(tokenMint: string): Promise<void> {
  try {
    ...
    console.log(
      `🚫 PERMANENT BAN COMPLETE: ${tokenMint} - will not be processed again`
    );
  } catch (error) {
