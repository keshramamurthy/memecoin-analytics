...
export class SocketService {
  ...
  private setupNamespace(): void {
    ...
    wsNamespace.on('connection', async (socket) => {
      ...
      socket.emit('connected', {
        ...
        socketId: socket.id,
        usage:
          'Send "tokenMint,subscribe" to subscribe or "tokenMint,unsubscribe" to unsubscribe',
      });
  ...
  private async handleSubscriptionMessage(
    ...
    try {
      ...
      const [tokenMint, action] = message
        .split(',')
        .map((s: string) => s.trim());
      ...
      if (!tokenMint || !action) {
        socket.emit('error', {
          message:
            'Invalid message format. Use "tokenMint,subscribe" or "tokenMint,unsubscribe"',
        });
        return;
      }
  ...
  private async subscribeToToken(
    ...
    try {
      ...
      if (!validation.isValid) {
        console.log(
          `Subscription rejected: invalid token ${tokenMint} - ${validation.reason}`
        );
  ...
  private async unsubscribeFromToken(
    socket: Socket,
    tokenMint: string
  ): Promise<void> {
    try {
  ...
  private handleDisconnection(socket: Socket): void {
    ...
    if (subscriptions) {
      console.log(
        `Socket ${socket.id} disconnected with ${subscriptions.subscriptions.size} active subscriptions`
      );
