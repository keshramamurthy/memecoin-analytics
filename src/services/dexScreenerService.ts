...
export class DexScreenerService {
  ...
  async getTokenPrice(
    tokenMint: string
  ): Promise<BatchTokenPriceResult | null> {
      ...
      try {
        ...
        for (const tokenMint of batch) {
          try {
            ...
            console.warn(
              `Failed to fetch single token price for ${tokenMint}:`,
              singleError
            );
          }
        }
  ...
  private async fetchBatchPrices(
    ...
    console.log(
      `🔍 Fetching batch prices from DexScreener: ${tokenMints.length} tokens`
    );
  ...
  private async fetchSingleTokenPrice(
    tokenMint: string
  ): Promise<BatchTokenPriceResult | null> {
    const url = `${this.baseUrl}/token-pairs/v1/${this.chainId}/${tokenMint}`;

    console.log(
      `🔍 Fetching single token price from DexScreener: ${tokenMint}`
    );
  ...
  private processPairsResponse(
    pairs: DexScreenerPair[],
    requestedTokens: string[]
  ): BatchTokenPriceResult[] {
    const results: BatchTokenPriceResult[] = [];

    console.log(
      `🔬 processPairsResponse: ${pairs.length} pairs received, ${requestedTokens.length} tokens requested`
    );
    pairs.forEach((p) =>
      console.log(
        `  Received pair: ${p.dexId} - $${p.priceUsd} (${p.baseToken.address}/${p.quoteToken.address})`
      )
    );
    ...
    console.log(
      `📊 Processing ${tokenPairsMap.size} tokens from DexScreener response`
    );
    for (const [tokenMint, tokenPairs] of tokenPairsMap) {
      console.log(
        `🎯 Processing token ${tokenMint}: ${tokenPairs.length} pairs found`
      );
      ...
      if (bestPair) {
        ...
        if (result) {
          results.push(result);
          console.log(
            `✅ DexScreener price for ${tokenMint}: $${result.priceUsd} (${result.priceInSol} SOL)`
          );
        }
      }
    }
  ...
  private selectBestPair(
    ...
    console.log(
      `🔍 Selecting best pair for ${tokenMint}: ${pairs.length} pairs available`
    );
    pairs.forEach((p) =>
      console.log(
        `  Available: ${p.dexId} - $${p.priceUsd} (L: $${p.liquidity?.usd || 0})`
      )
    );
    ...
    const qualityPairs = pairs.filter((p) => {
      ...
      console.log(
        `  ${p.dexId}: L=$${liquidityUsd.toFixed(2)} V=$${volume24h.toFixed(2)} Price=$${p.priceUsd}`
      );

      // Be very strict with suspected launchpad/pump pools first
      const dexIdLower = p.dexId.toLowerCase();
      const isLaunchpad =
        dexIdLower.includes('pump') ||
      ...
      if (isLaunchpad) {
        ...
        if (!hasHighVolume || !hasHighLiquidity) {
          console.log(
            `    ❌ Filtering out launchpad ${p.dexId}: V=$${volume24h} L=$${liquidityUsd}`
          );
          return false;
        }
      }
      ...
      const isEstablishedDex = ['raydium', 'orca', 'jupiter', 'meteora'].some(
        (dex) => p.dexId.toLowerCase().includes(dex)
      );
      ...
      if (isEstablishedDex) {
        ...
        if (liquidityUsd < MIN_LIQUIDITY_USD) {
          console.log(
            `Filtering out established DEX ${p.dexId}: low liquidity $${liquidityUsd}`
          );
          return false;
        }
      } else {
        ...
        if (volume24h < MIN_VOLUME_24H || liquidityUsd < MIN_LIQUIDITY_USD) {
          console.log(
            `Filtering out unknown DEX ${p.dexId}: V=$${volume24h} L=$${liquidityUsd}`
          );
          return false;
        }
      }
    ...
    if (qualityPairs.length === 0) {
      console.warn(
        `No quality pairs found for ${tokenMint}, using highest liquidity available`
      );
    ...
    let solPairs = establishedPairs.filter(
      (p) =>
        (p.baseToken.address === tokenMint &&
          (p.quoteToken.address === solMint ||
            p.quoteToken.address === wsolMint)) ||
        ((p.baseToken.address === solMint ||
          p.baseToken.address === wsolMint) &&
          p.quoteToken.address === tokenMint)
    );
    ...
    if (solPairs.length > 0) {
      ...
      console.log(
        `Selected SOL pair for ${tokenMint}: ${bestSolPair.dexId} with $${bestSolPair.liquidity?.usd?.toLocaleString() || 0} liquidity`
      );
      return bestSolPair;
    }

    let usdcPairs = establishedPairs.filter(
      (p) =>
        (p.baseToken.address === tokenMint &&
          p.quoteToken.address === usdcMint) ||
    ...
    if (usdcPairs.length > 0) {
      ...
      console.log(
        `Selected USDC pair for ${tokenMint}: ${bestUsdcPair.dexId} with $${bestUsdcPair.liquidity?.usd?.toLocaleString() || 0} liquidity`
      );
      return bestUsdcPair;
    }
    ...
    console.log(
      `Selected best pair for ${tokenMint}: ${bestPair.dexId} with $${bestPair.liquidity?.usd?.toLocaleString() || 0} liquidity`
    );
    return bestPair;
  }
  ...
  private calculatePairScore(pair: DexScreenerPair): number {
    ...
    const dexBonus = establishedDexes.some((dex) =>
      pair.dexId.toLowerCase().includes(dex)
    )
      ? 50000
      : 0;
    ...
    console.log(
      `Pair ${pair.dexId} score: ${finalScore.toFixed(0)} (L:${liquidityScore.toFixed(0)} V:${volumeScore.toFixed(0)} T:${txnScore.toFixed(0)} DEX:${dexBonus} LP:${launchpadPenalty} A:${activityBonus})`
    );

    return finalScore;
  }

  private convertPairToResult(
    pair: DexScreenerPair,
    tokenMint: string
  ): BatchTokenPriceResult | null {
    try {
      ...
      return {
        tokenMint,
        priceUsd,
        priceInSol: isNaN(priceInSol)
          ? priceUsd / SOL_PRICE_FALLBACK
          : priceInSol,
  ...
  async getSolPriceUsd(): Promise<number> {
    try {
      ...
      const solUsdcPair = response.data.find(
        (pair) =>
          (pair.baseToken.address === solMint &&
            pair.quoteToken.address === usdcMint) ||
          (pair.baseToken.address === usdcMint &&
            pair.quoteToken.address === solMint)
      );
